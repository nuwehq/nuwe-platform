...
describe V3::EatsController do
  ...
  before do
    ...
    FactoryGirl.create :capability, service_id: nuwe_nutrition_service.id, application_id: oauth_application.id
  end
  describe "create" do
    it "accepts components" do
      expect {
        post "/v3/eats.json", {eat: { components: [{ingredient_id: ingredient.id, amount: 70}], meal_ids: [meal.id] } }, bearer_auth
      }.to change {
        oauth_user.eats.count
      }.by(1)
    end

    it "accepts meal-ids" do
      expect {
        post "/v3/eats.json", {eat: {meal_ids: [meal.id]}}, bearer_auth
      }.to change {
        oauth_user.eats.count
      }.by(1)
    end
    ...
    it "accepts products" do
      expect {
        post "/v3/eats.json", {eat: {product_ups: [product.upc]}}, bearer_auth
      }.to change {
        oauth_user.eats.count
      }.by(1)
    end
    ...
    it "has a breakdown" do
      post "/v3/eats.json", {eat: {components: [{ingredient_id: carrots_raw.id, amount: 70}], meal_ids: [meal.id]}}, bearer_auth
  ...
  describe "index action" do
    ...
    it "shows al my eats" do
      get "/v3/eats.json", nil, bearer_auth
    ...
    it "is a paginated list" do
      3.times { FactoryGirl.create :eat, user: oauth_user }
      get "/v3/eats.json?per_page=1", nil, bearer_auth
  ...
  describe "today's index" do
    it "shows all eats for today" do
      Eat.destroy_all
      3.times { FactoryGirl.create :yesterdays_eat, user: oauth_user }
      3.times { FactoryGirl.create :eat, user: oauth_user }
      get "/v3/eats/today.json", nil, bearer_auth
  ...
  describe "edit" do
    ...
    it "returns a 200 status" do
      patch "/v3/eats/#{eat.id}.json", {eat: { components: [{ingredient_id: carrots_raw.id, amount: 70}], meal_ids: [meal.id] } }, bearer_auth
      expect(response.status).to eq(200)
    end

    it "contains correct ingredients" do
      patch "/v3/eats/#{eat.id}.json", {eat: { components: [{ingredient_id: turkey_whl_meat.id, amount: 240}]}}, bearer_auth
      expect(json_body["eat"]["breakdown"]["kcal_g"]).to eq("381.6")
    end

    it "can update an eaten product" do
      patch "/v3/eats/#{eat.id}.json", {eat: { product_upcs: [product.upc]}}, bearer_auth
  ...
  describe "delete" do
    it_behaves_like "a nice 404 response" do
      before do
        delete "/v3/eats/teamsnowden.json", nil, bearer_auth
      end
    end

    it "returns a 200 status" do
      delete "/v3/eats/#{eat.id}.json", nil, bearer_auth
      expect(response.status).to eq(200)
    end
  ...
  describe "spaghetti test" do
    it "has the correct breakdown" do
      post "/v3/eats.json", {eat: {components: [
  ...
  describe "eating products" do
    ...
    it "has the correct breakdown" do
      post "/v3/eats.json", { eat: {product_upcs: [product.upc]}}, bearer_auth
      expect(json_body["eat"]["breakdown"]["kcal_g"]).to eq("200.0")
  ...
  describe "eat locations" do
    it "can create lat lon" do
      post "/v3/eats.json", { eat: {lat: 52.352108, lon: 4.890718 }}, bearer_auth
      expect(json_body["eat"]["lat"]).to eq("52.352108")
      expect(json_body["eat"]["lon"]).to eq("4.890718")
    end
    it "can update the lat lon" do
      patch "/v3/eats/#{eat.id}.json", { eat: {lat: 53.352108, lon: 4.890718 }}, bearer_auth
      expect(json_body["eat"]["lat"]).to eq("53.352108")
    end
    it "can create places" do
      post "/v3/eats.json", { eat: {places: [{name: "First place", address: "26 Salisbury St Blandford Forum, Dorset DT11 7AT United Kingdom", lat: 52.352108, lon: 4.890718 }] }}, bearer_auth
      expect(json_body["eat"]["places"]).to_not be_empty
    end
    it "can create more than one place" do
      post "/v3/eats.json", { eat: {places: [{name: "First place", address: "26 Salisbury St Blandford Forum, Dorset DT11 7AT United Kingdom", lat: 52.352108, lon: 4.890718},{name: "Second place", address: "26 Salisbury St Blandford Forum, Dorset DT11 7AT United Kingdom", lat: 52.352108, lon: 4.890718}]}}, bearer_auth
      expect(json_body["eat"]["places"]).to_not be_empty
    end
    it "can update places" do
      patch "/v3/eats/#{eat.id}.json", { eat: {places: [{name: "Updated place", address: "26 Salisbury St Blandford Forum, Dorset DT11 7AT United Kingdom", lat: 52.352108, lon: 4.890718 }] }}, bearer_auth
      expect(json_body["eat"]["places"]).to_not be_empty
      expect(response.status).to eq(200)
    end
  end
