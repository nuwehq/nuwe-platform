IntercomRails.config do |config|
  ...
  config.user.custom_data = {
    :authorized_client_apps => Proc.new { |user| Doorkeeper::AccessGrant.where(resource_owner_id: user.id).map { |token| token.application.id } },
    :is_developer => Proc.new { |user| user.roles.include?("developer") },
    :developer_apps => Proc.new { |user| user.oauth_applications.count },
    :collaborations => Proc.new { |user| user.collaborated_applications.count }
  }
